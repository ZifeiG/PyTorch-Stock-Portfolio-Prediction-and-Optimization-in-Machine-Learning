...
def estimate_covariance_from_window(
    ...
    # scale Sigma by trace to improve conditioning (typical trick for cone solvers)
    # tr = np.trace(Sigma); if tr > 0: Sigma = Sigma / tr
    return Sigma


# ------------------------------------------------------------------
# Markowitz solvers (sum(x) <= 1, no cap)
# ------------------------------------------------------------------
def _solve_markowitz_socp(mu_hat: np.ndarray, Sigma: np.ndarray, delta_risk: float,
    ...
    Prefer solver in order: CLARABEL -> ECOS -> SCS.
    """
    n = mu_hat.size
    x = cp.Variable(n)
    ...
    # if not solved or no value, try ECOS
    if (x.value is None) or (prob.status not in ("optimal", "optimal_inaccurate")):
        try:
            prob.solve(solver=cp.ECOS, verbose=False,
                       max_iters=2000, abstol=1e-8, reltol=1e-8, feastol=1e-8,
                       warm_start=True)
        except Exception:
            pass
    ...
    if (x.value is None) or (prob.status not in ("optimal", "optimal_inaccurate")):
        try:
            prob.solve(solver=cp.SCS, verbose=False,
                       max_iters=30000, eps=5e-6, acceleration_lookback=20,
                       use_indirect=True)
        except Exception:
            pass
    ...
    s = float(w.sum())
    return (w / s) if s > 1e-12 else np.ones(n) / nv



def _solve_markowitz_qp_osqp(mu_hat: np.ndarray, Sigma: np.ndarray, gamma: float) -> np.ndarray:
    """
    OSQP needs QP with linear constraints only. Use mean-variance objective:
      max  mu^T x - gamma * x^T Σ x
      s.t. sum(x)=1, x>=0
    NOTE: This is an alternative to the risk-budget constraint; not equivalent but useful/robust.
    """
    n = mu_hat.size
    ...
    try:
        prob.solve(solver=cp.OSQP, verbose=False,
                   max_iter=20000, eps_abs=1e-8, eps_rel=1e-8, polish=True)
    except Exception:
        return np.ones(n) / n
    if x.value is None:
        return np.ones(n) / n
...
def solve_markowitz(mu_hat: np.ndarray,
    ...
    """
    Wrapper:
      - "SOCP_AUTO": use CLARABEL->ECOS->SCS chain on constraint x^T Σ x <= δ
      - "OSQP_QP":   use mean-variance objective for OSQP (no risk constraint)
    """
...
# ------------------------------------------------------------------
# Training + Evaluation (dataset/loader now passed in)
# ------------------------------------------------------------------
def train_and_eval_spo(loader: DataLoader,
                       ...
                       solver_choice: str = "SOCP_AUTO",
                       gamma_mv: float = 5.0,
                       max_updates: int | None = None, # limit # of optimizer steps (1~2) if set
                       mode: str = "train"):

    # infer num_stocks from a batch sample
    ...
    for epoch in range(epochs):
        ...
        for xb, tb, yb, sb in loader:
            ...
            for i in range(mu_pred.size(0)):
                ...
                # ===== Evaluation using the same solver chain but tighter δ if needed =====
                model.eval()
                with torch.no_grad():
            ...
            if valid_cnt > 0:
                ...
                # ---- early stop after limited updates ----
        ...
        print(f"[{mode.upper()}] Epoch {epoch + 1}/{epochs}, Loss={epoch_loss / max(len(loader), 1):.6f}")
        ...
        if len(all_records) > 0:
            ...
            print(f"   [Debug] max weight={max_w:.4f} (asset #{argmax_w}), mean={mean_w:.4e}")
            if max_w > 0.9:
        ...
        save_path = os.path.join(save_dir, f"Markowitz_SPO+_{mode}_epoch{epoch + 1}.csv")
        df_tmp.to_csv(save_path, index=False, encoding="utf-8-sig")
        print(f"Intermediate {mode} results saved to: {save_path}\n")

        # ----------------------------------------
        # Save checkpoints
        # Save the model every 5 epochs, prevent the loss of the model in case of interruption during training.
        # ----------------------------------------
        if (epoch + 1) % 5 == 0 or (epoch + 1) == epochs:
            ckpt_path = os.path.join(save_dir, f"{mode}_checkpoint_epoch{epoch + 1}.pt")
            torch.save(model.state_dict(), ckpt_path)
            print(f"Model checkpoint saved: {ckpt_path}\n")

    # Save final results
    final_path = os.path.join(save_dir, f"Markowitz_SPO+_{mode}_final.csv")
...
if __name__ == "__main__":
    ...
    train_path = os.path.join(base_dir, "Raw Datasets", "train_NYSE.csv")
    test_path  = os.path.join(base_dir, "Raw Datasets", "test_NYSE.csv")
    ...
    # Training
    # ==============================================================
    new_save_dir = os.path.join(os.getcwd(), "Integration Summary_3")
    ...
    dataset_t = TensorDataset(torch.tensor(X_last_t, dtype=torch.float32),
    ...
    train_csv = os.path.join(new_save_dir, "Markowitz_SPO+_train_final.csv")
    test_csv  = os.path.join(new_save_dir, "Markowitz_SPO+_test_final.csv")
