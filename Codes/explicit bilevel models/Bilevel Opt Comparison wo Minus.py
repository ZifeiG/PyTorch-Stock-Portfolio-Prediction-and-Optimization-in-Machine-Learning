# ================================================================
# Compare KKT vs closed-form u* (no KKT) w/o cost(minus term)
# ================================================================
        ...
        for xb, yb, sb in loader:
            ...
            with torch.no_grad():
                ...
                for i in range(d_pred_np.shape[0]):
                    ...
                    # Evaluate regret (w/o cost-based)
                    ...
                    pred_loss_kkt = (rho * np.sum(u_eval_kkt)) / denom
                    pred_loss_ustar = (rho * np.sum(u_eval_ustar)) / denom
                    oracle_loss = (rho * np.sum(u_eval_star)) / denom

                    regrets_kkt.append(pred_loss_kkt - oracle_loss)
                    regrets_ustar.append(pred_loss_ustar - oracle_loss)
                    ...
                    all_rows.append({
                        "epoch": int(epoch),
                        "index": int(sb_np[i]),
                        "regret_kkt": float(pred_loss_kkt - oracle_loss),
                        "regret_ustar": float(pred_loss_ustar - oracle_loss),
...
if __name__ == "__main__":
    ...
    save_dir = os.path.join(os.getcwd(), "Bilevel Opt Summary wo Minus")
