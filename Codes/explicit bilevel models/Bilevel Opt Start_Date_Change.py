# ================================================================
# Compare KKT vs closed-form u* (no KKT) under different starting date
# ================================================================
...
def train_compare(loader, gross_data, cost_vec, amax, rho, epochs, lr, save_dir, cap_factor):
    ...
    out_path = os.path.join(save_dir, "bilevel_comparison_final.csv")
...
if __name__ == "__main__":
    ...
    gross_full = pd.read_csv(data_path).values
    T_full, n = gross_full.shape

    # choose several starting indices
    start_list = [0, 250, 500]  # Changing

    for start_idx in start_list:
        print(f"\n========== Start index = {start_idx} ==========")
        # Extract the subsequence starting from start_idx
        gross = gross_full[start_idx:]
        T, n = gross.shape
        ...
        # Re-calculations: amax, c, rho, cap_factor
        total_demand = demand.sum(axis=1)
        amax = float(np.quantile(total_demand, 0.20))   # still 20th percentile
        amax = max(1.0, amax)
        ...
        c = base_c * (1.0 + 0.5 * (vol_norm - 1.0))
        c = np.clip(c, 0.05, 0.5)

        rho = 0.6

        print(
            f"[INFO] start_idx={start_idx}, scale={scale:.4e}, "
            f"amax={amax:.4f}, c_mean={c.mean():.3f}, "
            f"rho={rho}, cap_factor={cap_factor}"
        )

        # Construct the DataLoader corresponding to this sub-interval
        ...
        save_dir = os.path.join(
            os.getcwd(),
            f"Bilevel Opt Summary Start",
            f"start_{start_idx}"
        )

        train_compare(loader, gross, c, amax, rho,
                      epochs=10, lr=1e-3,
                      save_dir=save_dir, cap_factor=cap_factor)
